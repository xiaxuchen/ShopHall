...
import com.cxyz.http.exception.OKHttpException;
import com.cxyz.http.listener.DisposeDataHandler;
import com.cxyz.http.listener.DisposeDownLoadListener;
...
public class CommonFileCallback implements Callback {
    ...
    public void onResponse(Call call, Response response) throws IOException {
        //当状态码异常时直接回调失败
        if(response.code() == UNPROCESSABLE) {
            mDeliveryHandler.post(() -> {
                mListener.onFailure("找不到资源");
            });
            return;
        }
